...
class _QuestionnaireScreenState extends State<QuestionnaireScreen> {
  ...
  Map<String, int> counts = {"Dry": 0, "Normal": 0, "Oily": 0, "Combination": 0};
  for (var answer in _answers) {
    if (answer != null) counts[answer] = counts[answer]! + 1;
  }

  // 1. Determine base type
  String finalSkinType = counts.entries.reduce((a, b) => a.value > b.value ? a : b).key;

  // 2. Determine status based on the quiz data (inferred insights)
  ...
  switch (finalSkinType) {
    ...
    case "Normal":
      ...
      break;
    case "Oily":
      inferredStatus = "Dehydrated (Needs water, not oil)";
      break;
    case "Combination":
  ...
  if (mounted) _showResultDialog(finalSkinType);
 }
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.stars, color: Color(0xFFA259B3), size: 50),
              ),
              const SizedBox(height: 20),
              const Text("Assessment Complete!", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 10),
              Text("Your baseline skin type is:", style: TextStyle(color: Colors.grey.shade600)),
              const SizedBox(height: 5),
              Text(skinType.toUpperCase(), style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Color(0xFFA259B3), letterSpacing: 1.5)),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Skin Assessment", style: TextStyle(color: Colors.black87, fontSize: 18)),
        centerTitle: true,
      ),
      body: Column(
        children: [
